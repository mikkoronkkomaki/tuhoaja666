...
* [`src/tuhoaja666/`](src/tuhoaja666/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/tuhoaja666/core.cljs): contains the SPA entry point, `init`
...
The `debug?` variable in [`config.cljs`](src/cljs/tuhoaja666/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tuhoaja666.example
  (:require [tuhoaja666.config :as config])
